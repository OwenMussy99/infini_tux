...
public class LevelScene extends Scene implements SpriteContext
{
    ...
    static int highScore; // Highscore variable
    ...
    public static void getHighscore() throws IOException {
    	// The below code gets the highscore from the highscore.txt file. This doesn't work whem the code is exported
    	// to a jar. A total of 11hrs has been put into trying to figure out why that was happening, and nothing worked.
    	// Tried to get the abs path of the jar file and read the highscore.txt file from the directory the jar was in,
    	// but that didn't work, along with trying to pass command line arguments using an input file or just numbers, 
    	// nothing worked.
    	try {
    		if (retrieveHighcore == true) {
    			...
    			BufferedReader highScoreReader = new BufferedReader(new FileReader(hsFile));
    ...
    public static void updateHighscore(int score) {
    	try {
        	...
        	String highScorePath = file2.getAbsolutePath();
    ...
    public void init()
    {
        ...
        //Create default directories if they don't exist already
